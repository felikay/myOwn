...
class sellerController extends Controller
{
  ...
  public function sellerEndedAuction()
  {
      ...
      $data = Posted::where('seller_email', $userId)
          ->where('end_time', '<=', Carbon::now()->timezone('Africa/Nairobi'))
          ->get();
  
      // Fetch the highest bid and bidder_email for each expired item
      foreach ($data as $product) {
          $highestBid = DB::table('bids')
              ->where('product_id', $product->product_id)
              ->where('amount', DB::raw("(SELECT MAX(amount) FROM bids WHERE product_id = {$product->product_id})"))
              ->select('bidder_email', 'amount')
              ->first();
  
          $product->highest_bidder_email = $highestBid ? $highestBid->bidder_email : 'No Bidder';
          $product->highest_bid_amount = $highestBid ? $highestBid->amount : 'No Amount';
      }
